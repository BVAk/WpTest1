...
function hexagon_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control('hexagon_slider_hide_show',array(
	   	...
	   	'description' => __('Image Size ( 1500 x 700 )','hexagon'),
	...
	for ( $count = 1; $count <= 4; $count++ ) {
		...
		$wp_customize->add_setting( 'hexagon_slider' . $count, array(
			'default'           => '',
			'sanitize_callback' => 'hexagon_sanitize_dropdown_pages'
		) );

		$wp_customize->add_control( 'hexagon_slider' . $count, array(
			'label'    => __( 'Select Slide Image Page', 'hexagon' ),
			'section'  => 'hexagon_slider_section',
			'type'     => 'dropdown-pages'
		) );
	}
		...
		$manager->add_section(
			new Hexagon_Customize_Section_Pro(
				...
				array(
					...
					'pro_url'  => esc_url( 'https://www.luzuk.com/themes/hexagon-wordpress-theme/' ),
				)
			)
		);
	}
